...
static void esp_xiaozhi_chat_app_audio_read(void *pv)
{
    ...
    while (true) {
        ret = audio_recorder_read_data(s_recorder, data, ESP_XIAOZHI_CHAT_REC_READ_SIZE);
        if (ret > 0) {
...
static esp_err_t esp_xiaozhi_chat_app_audio(esp_xiaozhi_chat_app_t *xiaozhi_chat_app, esp_board_manager_adapter_info_t bsp_info)
{
    ...
    afe_config.mode.afe.algo_mask |= AV_PROCESSOR_AFE_FLAG_AI_MODE_WAKEUP;
    ...
    config.play_io.write_cb = board_audio_write_cb;
    config.play_io.write_ctx = bsp_info.play_dev;
    ...
    do {
        ...
        if (esp_codec_dev_set_out_vol((esp_codec_dev_handle_t)bsp_info.play_dev, 60) != ESP_CODEC_DEV_OK) {
            ret = ESP_FAIL;
            ESP_LOGE(TAG, "Failed to set output volume");
            break;
        }
        if (esp_codec_dev_set_in_gain((esp_codec_dev_handle_t)bsp_info.rec_dev, 26.0) != ESP_CODEC_DEV_OK) {
            ret = ESP_FAIL;
        ...
        ret = audio_recorder_open(&recorder_config, &s_recorder);
        if (ret != ESP_OK) {
        ...
        ret = audio_feeder_open(&feeder_config, &s_feeder);
        if (ret != ESP_OK) {
    ...
    if (ret != ESP_OK) {
        ...
        if (recorder_opened) {
            audio_recorder_close(s_recorder);
        }
        if (playback_opened) {
